expertManage2 <- function(now, rdb = NULL, v = T) {
  ...
  if (sum(old.wts)>0) {
    pf.rets <- expertRFPredict2(now, exp.wts = old.wts, rdb = rdb)
  } else {
    pf.rets <- expertRFPredict2(now, exp.wts = NULL, rdb = rdb)
  }
  
  # Get the return of each expert's L-S portfolio
  exp.rets <- as.numeric(pf.rets[1,2:(length(expnf$eid)+1)])
  # note("expertManage: avg rts:", round(pf.rets$avg,2), lvl=1)
  # note("expertManage: exp rts:", round(pf.rets$exp,2), lvl=1)
  ...
  cur.perf <- as.data.frame(cbind(now, c(expnf$eid, "avg", "exp", "expc", "avg2"), as.numeric(pf.rets[,-1]) ))
  ...
  # Record the time
  note("expertManage: runtime =", (proc.time() - pt)[1], lvl=2)
}
